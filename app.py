...
def render_terminal_sidebar() -> tuple[str, list[str], str]:
    ...
    The analysis engines remain unchanged; only the visible navigation is reduced.
    Showing one workspace at a time also prevents duplicate Streamlit widget keys.
    """
    st.sidebar.markdown(
        """
        <div class="sidebar-brand">
            <div class="sidebar-logo">PSX</div>
            <h3>Simple Decision Terminal</h3>
            <p>Five main workspaces. All technical patterns, indicators, uploaded PDFs, and saved fundamental knowledge remain connected to the analysis engine.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    selected_page = st.sidebar.radio(
        ...
        main_pages,
        index=1,
        key="simple_main_page",
        help="Only one workspace opens at a time, keeping the bot clean and fast.",
    )

    st.sidebar.markdown(
        """
        <div class="sidebar-note">
            <strong>Analysis kept active:</strong><br>
            Candlestick patterns, chart patterns, RSI/MACD divergence, moving averages,
            support/resistance, volume, ATR, risk rules, fundamentals, and uploaded PDF knowledge.<br><br>
            Use <strong>Data & Settings</strong> only when uploading or updating information.
        </div>
        """,
        unsafe_allow_html=True,
    )
    return theme_name, [selected_page], "Single workspace"
    ...
    st.markdown(
        ...
        <div class="terminal-shell">
            <div class="terminal-header">
                ...
                <h1>Simple PSX Decisions. Full Analysis Behind the Scenes.</h1>
                <p>Active desks: {", ".join(active_modules[:5])}{" ..." if len(active_modules) > 5 else ""}. Use the five main workspaces for alerts, decisions, scenarios, divergences, and portfolio checks.</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
st.caption(
    "Simple mode: one workspace opens at a time. The full pattern, indicator, fundamental, and uploaded-PDF knowledge engines remain active."
)
...
def scenario_finder_simple_panel() -> None:
    st.subheader("Scenario Finder")
    st.caption("Find trade scenarios across selected symbols, universes, and timeframes. Full pattern and indicator checks remain active.")
    scenario_scanner_panel()


def portfolio_checker_simple_panel() -> None:
    st.subheader("Portfolio Checker")
    st.caption("Upload or connect your portfolio, then review Hold, Buy More, Average, Reduce, Sell, risk, and diagnostics.")
    autopilot_manager_panel()


def data_settings_simple_panel() -> None:
    st.subheader("Data & Settings")
    st.info("This supporting page is only for data uploads and knowledge updates. Uploaded PDFs and pattern material are retained for use across the five main workspaces.")
    with st.expander("Uploaded PDFs & Pattern Knowledge", expanded=True):
        uploaded_knowledge_brain_panel()
    with st.expander("Sarmaaya / Fundamental Data", expanded=False):
        sarmaaya_data_import_center_panel()
    with st.expander("Fundamental Image Import", expanded=False):
        fundamental_image_import_center_panel()


PAGE_RENDERERS = {
    "1. Fast Up & Down Alert": fast_down_alert_scanner_panel,
    "2. Decision Center": decision_center_panel,
    "3. Scenario Finder": scenario_finder_simple_panel,
    "4. Divergence Finder": latest_divergence_scanner_desk_panel,
    "5. Portfolio Checker": portfolio_checker_simple_panel,
    "Data & Settings": data_settings_simple_panel,
}
...
def render_workspace_panels(active_panels: list[str], layout_mode: str) -> None:
    selected = active_panels[0] if active_panels else "2. Decision Center"
    renderer = PAGE_RENDERERS.get(selected, decision_center_panel)
    st.markdown(
        f"""
        <div class="terminal-panel">
            <span class="terminal-chip chip-green">Simple Mode</span>
            <span class="terminal-chip chip-blue">{selected}</span>
            <span class="terminal-chip chip-amber">Full analysis engine active</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
